...
// Steinhart-Hart方程系数 (100K NTC典型值)
const float A = 0.003354016;   // Steinhart-Hart系数A
const float B = 0.000256524;   // Steinhart-Hart系数B
const float C = 0.000003508;   // Steinhart-Hart系数C
...
void setup() {
  ...
  Serial.println("设置: 增益=±4.096V, 采样率=128SPS");
...
float readAveragedADC(int channel) {
    ...
    if (validSamples < SAMPLES/2) {
        Serial.println("ADC读取错误: 有效样本不足");
        return -1;
    }
    
    float average = sum / validSamples;
    Serial.printf("ADC平均值: %.2f (有效样本: %d/%d)\n", average, validSamples, SAMPLES);
    Serial.println("=== ADC读取结束 ===\n");
...
float calculateNtcResistance(float adcValue) {
    ...
    // ADC值有效性检查
    if (adcValue < 0) {
        Serial.println("无效的ADC值");
        return -1;
    }
    
    // 计算实际电压 (使用4.096V量程)
    float voltage = (adcValue / 32767.0) * 4.096;
    Serial.printf("ADC值: %.0f, 电压: %.3fV\n", adcValue, voltage);
    
    // 电压范围检查
    if (voltage < 0.1 || voltage > 4.0) {
        Serial.printf("电压超出范围: %.3fV\n", voltage);
        return -1;
    }
    
    // 计算NTC电阻 - 修正分压公式
    float ntc_resistance = R1 * voltage / (3.3 - voltage);  // 修改这里的分压计算
    Serial.printf("计算得到的NTC电阻: %.0f Ω\n", ntc_resistance);
    
    // 电阻范围检查 (100K NTC通常在100Ω到1MΩ之间)
    if (ntc_resistance < 100 || ntc_resistance > 1000000) {
        Serial.printf("电阻值异常: %.0f Ω\n", ntc_resistance);
        return -1;
    }
    
    Serial.println("=== 电阻计算结束 ===\n");
...
float calculateTemperature(float resistance) {
    ...
    // 电阻值有效性检查
    if (resistance < 0) {
        Serial.println("无效的电阻值");
        return -999.0;
    }
    
    // 使用Beta方程计算温度
    float temperature = 1.0 / (1.0/T0 + (1.0/BETA) * log(resistance/R0)) - 273.15;
    
    Serial.printf("电阻: %.0f Ω, 温度: %.1f°C\n", resistance, temperature);
    
    // 温度范围检查 (-55到125℃是常见NTC的工作范围)
    if (temperature < -55 || temperature > 125) {
        Serial.printf("温度超出范围: %.1f°C\n", temperature);
        return -999.0;
    }
    
    Serial.println("=== 温度计算结束 ===\n");
    return temperature;
}
